...
CONSTRAINT_SEMANTICS_ZH = {
    ...
    "inner_transport_modes_subset": "实际使用的所有市内交通方式必须是给定允许集合的子集；不要求集合中的每种方式都出现。",
    "taxi_cars": "只要使用出租车，每次使用的车辆数都必须等于指定值。",
    "intercity_modes_include": "实际城际交通方式集合必须包含给定的每一种方式。",
    "inner_transport_mode_count": "按每个活动的完整入站市内交通链分类，指定交通方式的行程次数必须恰好等于给定值。",
    "walking_distance_budget": "所有市内交通链中的步行路段距离之和不得超过指定公里数。",
    "innercity_travel_time_budget": "所有市内交通路段的持续时间之和不得超过指定分钟数。",
    ...
    "innercity_budget": "所有市内交通段费用之和不得超过指定上限。",
    ...
    "forbidden_inner_transport_modes": "实际使用的市内交通方式不得命中给定集合。",
